/**
 * Replace expandOne in app.js with a version that has comprehensive emotional/human mapping.
 * Run this script via: node replace_emotional2.js
 */
const fs = require('fs');
...
// Find expandOne function completely using brace counting.
...
// New expandOne function with a much richer emotional mapping.
const newFunc = `function expandOne(row, fallbackId) {
  if (!row) return null;
  if (!Array.isArray(row)) return row;
  const topic = row[2] || "life";
  // Comprehensive emotional/human mapping for option labels and hints.
  const emotionMap = {
    // SN axis - full conversational replacements
    "步骤拆解":"一步一步列清楚，别糊弄我",
    "方向画面":"先想想最终啥样，别瞎忙",
    "细节清单":"列个清单对核对，别飘",
    "概念草稿":"脑洞大开先画草图",
    ...
    "铃":"开始铃和结束铃别超时",
    "能量":"跟能量走有电就干没电就休",
    // EI axis extra conversational variants (humorous, relatable)
    "开麦怼回去":"直接开麦怼回去谁怕谁！",
    ...
    "跳过剧情":"只回必要信息当它是可跳过剧情",
    // Additional fallback tags that might appear from older generation
    "软着陆":"尽量让场面软着陆",
    "保人设":"顺便保住人设",
    "低消耗":"用最小社交成本",
    "可复盘":"还要能发朋友圈复盘",
...
console.log('expandOne replaced successfully with comprehensive emotional mapping.');
";
fs.writeFileSync('replace_emotional2.js', new Function('return ' + (new Function('return '` + `/**
 * Replace expandOne in app.js with a version that has comprehensive emotional/human mapping.
 * Run this script via: node replace_emotional2.js
 */
const fs = require('fs');
